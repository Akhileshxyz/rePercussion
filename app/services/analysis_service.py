    ...
    try:
        ...
        for url in preview_urls:
            try:
                ...
                rms_vals.append(float(np.mean(rms)))
                zcr = librosa.feature.zero_crossing_rate(y)
                zcr_vals.append(float(np.mean(zcr)))
                flat = librosa.feature.spectral_flatness(y=y)
                flatness_vals.append(float(np.mean(flat)))
                roll = librosa.feature.spectral_rolloff(y=y, sr=sr)
                rolloff_vals.append(float(np.mean(roll)))
                onset_strength = librosa.onset.onset_strength(y=y, sr=sr)
                onset_vals.append(float(np.mean(onset_strength)))
                chroma = librosa.feature.chroma_cqt(y=y, sr=sr)
                chroma_profiles.append(np.mean(chroma, axis=1).tolist())
            finally:
                try:
                    os.unlink(tmp_path)
                except Exception:
        ...
        if chroma_profiles:
            avg_chroma = np.mean(np.array(chroma_profiles), axis=0)
    ...
    try:
        ...
        if librosa_features:
            ...
            if "onset_mean" in librosa_features:
                ...
                if onset > 0.2:
                    feature_implications += "- Strong onsets (>0.2) suggests drums, piano, plucked string instruments (guitar, harp), or staccato playing techniques\n"
                else:
                    feature_implications += "- Weaker onsets (<0.2) suggests sustained instruments like strings, pads, ambient synthesizers, or legato playing techniques\n"
